...
describe('buildSchedule', () => {
  it('with no pauses, is one play segment over the whole timeline', () => {
    const { segments, totalDurationMs } = buildSchedule(26, 1000, [])
  ...
  it('pause at frame 0 holds first (no play before it)', () => {
    const { segments } = buildSchedule(10, 500, [pause('a', 0, 300)])
  ...
  it('merges duplicate-frame pauses by summing their durations', () => {
    const { segments } = buildSchedule(10, 500, [
      pause('a', 5, 200),
      pause('b', 5, 300),
    ])
  ...
  it('drops non-positive-duration pauses', () => {
    const withZero = buildSchedule(10, 500, [pause('a', 5, 0), pause('b', 3, -100)])
    const without = buildSchedule(10, 500, [])
  ...
  it('clamps out-of-range pause frames into [0, frameCount-1]', () => {
    const { segments } = buildSchedule(10, 500, [pause('a', 99, 100)])
  ...
  it('degenerate frameCount <= 1 yields no segments', () => {
    expect(buildSchedule(1, 500, [pause('a', 0, 100)])).toEqual({
      segments: [],
      totalDurationMs: 500,
    })
  })
